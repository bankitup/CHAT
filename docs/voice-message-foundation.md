...
- keep both committed transport-source lookup and local playable-source preparation behind that same file, with `resolveThreadVoicePlaybackSourceSnapshot(...)` for read-time state and `prepareThreadVoicePlaybackSource(...)` for player-facing preparation, so the thread UI does not own plaintext-vs-private media branching
