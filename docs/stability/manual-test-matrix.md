...
| Layout | Messenger bottom-nav fit | On narrow mobile width with Messenger shell navigation visible, switch between inbox, chat, and activity/home-adjacent surfaces | Bottom-nav shell stays centered and bounded, labels truncate instead of wrapping, and the active state does not distort neighboring tabs | `/app/(app)/app-shell-frame.tsx`, `/app/globals.css` |
